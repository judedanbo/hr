...
<template>
  ...
  <main >
    ...
    <div class="rounded-lg bg-gray-50 dark:bg-gray-500 shadow-sm ring-1 ring-gray-900/5 dark:ring-gray-300/80">
      ...
      <table v-if="transfers.length > 0" class="min-w-full">
        ...
        <thead class="border-b border-gray-300 text-gray-900 dark:text-gray-50">
          <tr>
            <th scope="col" class="py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-gray-900 dark:text-gray-50 sm:pl-0">Position</th>
            <th scope="col" class="hidden px-3 py-3.5 text-right text-sm font-semibold text-gray-900 dark:text-gray-50 sm:table-cell">Start</th>
            <th scope="col" class="hidden px-3 py-3.5 text-right text-sm font-semibold text-gray-900 dark:text-gray-50 sm:table-cell">End</th>
        ...
        <tbody>
          <tr v-for="transfer in transfers" :key="transfer.id" class="border-b border-gray-200">
            <td class="max-w-0 py-2 pl-1 pr-3 text-sm sm:pl-0">
              <div class="font-medium text-gray-900 dark:text-gray-50">{{ transfer.name }}</div>
              <div class="mt-1 truncate text-gray-500 dark:text-gray-100">{{ transfer.remarks }}</div>
            </td>
    ...
    <Modal @close="toggleTransferModal()" :show="openTransferModal">
      <Transfer @formSubmitted="toggleTransferModal()" :staff="staff" :units="units" :transfers="transfers" />
    </Modal>
  </main>
</template>
