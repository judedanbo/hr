      ...
      <table v-if="qualifications.length > 0" class="min-w-full">
        ...
        <thead class="border-b border-gray-300 dark:border-gray-200/50 text-gray-900 dark:text-gray-50">
          <tr>
            ...
            <th scope="col" class="hidden px-3 py-3.5 text-sm font-semibold text-gray-900 dark:text-gray-50 sm:table-cell">Course</th>
            <th scope="col" class="hidden px-3 py-3.5 text-sm font-semibold text-gray-900 dark:text-gray-50 sm:table-cell">Qualification</th>
            <th scope="col" class="hidden px-3 py-3.5 text-sm font-semibold text-gray-900 dark:text-gray-50 sm:table-cell">Year</th>
        ...
        <tbody>
          <tr v-for="qualification in qualifications" :key="qualification.id" class="border-b border-gray-200 dark:border-gray-400/30">
            ...
            <td class="hidden px-1 py-5 text-sm text-gray-500 dark:text-gray-100 sm:table-cell">{{ (qualification.level) }}</td>
            <td class="hidden px-1 py-5 text-sm text-gray-500 dark:text-gray-100 sm:table-cell">{{ (qualification.course) }}</td>
            
           
            <td class="hidden px-1 py-5 text-sm text-gray-500 dark:text-gray-100 sm:table-cell">{{ (qualification.qualification) }}
            <div class="font-medium text-xs text-gray-900 dark:text-gray-50">{{ qualification.qualification_number }}</div></td>
            <td class="hidden px-1 py-5 text-sm text-gray-500 dark:text-gray-100 sm:table-cell">{{ (qualification.year) }}</td>
                     </tr>
        </tbody>
       
      </table>
      <div v-else class="px-4 py-6 text-sm font-bold text-gray-400 dark:text-gray-100 tracking-wider text-center ">No qualifications found.</div>
    </div>
      </dl>
