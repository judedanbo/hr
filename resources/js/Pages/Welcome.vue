...
<template>
    ...
    <div class="relative flex items-top justify-center min-h-screen bg-gray-100 dark:bg-gray-900 items-center sm:pt-0">
        <div class="lg:flex flex-wrap w-full md:max-w- sm:px-6 lg:px-0 max-w-6xl">
            <img class="mx-auto lg:w-1/3" :src="logo" alt="Audit Service" width="150" />
            <div
                ...
                <h1 class="text-white lg:text-green-900 dark:text-gray-50 text-4xl hidden md:block">
                    Human Resource Management Information System
                </h1>
                <h1 class="text-white text-4xl md:hidden">HRMIS</h1>
                <Link v-if="$page.props.auth.user" :href="route('dashboard')" as="button"
                ...
                Home</Link>
                <Link v-else :href="route('login')" as="button"
                    class="mt-12 text-gray-700 lg:text-white text-2xl lg:text-3xl bg-white lg:bg-green-800 hover:bg-green-900 hover:text-white focus:outline-none hover:ring-green-600 rounded-lg px-8 py-3.5 text-center tracking-widest">
                login
                </Link>
                <div class="mt-2">
